...
## 스토어 스크린샷 8장으로 재작업 (2026-09-20, 유저 요청)
5장 → Play 최대치인 8장으로 확장 + 이름 변경 반영해서 재촬영 + 디자인 스킬로 문구 재작성.
설치 결정에 미치는 영향이 큰 순서로 배열: 핵심가치(1) → 진입장벽 해소(2, 붙여넣기) →
차별화 인사이트(3, 실질시급) → 개인화(4 슬라이더/5 템플릿) → 상세비교(6) → 스마트필터(7) →
신뢰 마감(8, 서버 없음).
- [x] `frame.html`에 8프레임 정의 + 화면별 맞춤 인터랙션 추가:
      - 3번(실질시급): 상단 nav/질문/템플릿 숨기고 순위카드+근거문장만 위로 당김
      - 5번(템플릿): 크롭 대신 실제로 "돈 우선형" 버튼을 클릭해 순위가 dynamically 바뀐
        결과(97점, 재정렬)를 그대로 캡처 — 크롭보다 "버튼 하나로 바뀐다"는 걸 더 설득력 있게 보여줌
      - 8번(프라이버시): 1위 카드만 남기고 백업/공유 버튼+개인정보처리방침 링크를 그 아래로 당겨와
        "실제로 존재하는 신뢰 증거"로 마무리
      **실수 하나 발견·수정**: `iframe.style.transform`으로 스케일을 기본값(1.8204) 밖으로 바꾸면
      412px 콘텐츠가 750px device box 폭을 넘어 오른쪽이 잘림(가로 스크롤 없는 캔버스라 크롭됨).
      모든 프레임은 스케일을 기본값 그대로 두고 세로 요소만 숨기거나 실제 인터랙션으로 콘텐츠를
      바꾸는 쪽으로 접근해야 가로 잘림이 안 생긴다.
- [x] 파일명 우선순위 반영: `01-ranking-hero.png` ~ `08-privacy-local.png` (기존 01~05 삭제)
- [x] `play-store-listing.md`에 8장 우선순위 표 갱신
검증: 1080x1920 PNG 8장 전부 agent-browser로 직접 렌더링해 눈으로 확인, 가로 잘림/빈 공간
과다 등 초기 시도의 결함 2건(스케일 오버라이드 잘림, 프라이버시 프레임 과도한 여백) 발견해 수정.
